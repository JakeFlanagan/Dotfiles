...
REPO_DIR="$HOME/Projects/Github/Dotfiles"
...
# ── Nerd Fonts (MesloLGS NF for Powerlevel10k) ───────────────────────────────
install_fonts() {
  # macOS uses a different font dir; skip and let the user handle it
  if [ "$(uname -s)" = "Darwin" ]; then
    warn "macOS detected, install MesloLGS NF manually via Homebrew Cask or Font Book."
    return
  fi
  ...
  if fc-list 2>/dev/null | grep -qi "MesloLGS NF"; then
    warn "MesloLGS NF already installed, skipping."
    return
  fi
  ...
  local styles=("Regular" "Bold" "Italic" "Bold%20Italic")
  local names=("Regular" "Bold" "Italic" "Bold Italic")

  for i in "${!styles[@]}"; do
    local url="${BASE_URL}/MesloLGS%20NF%20${styles[$i]}.ttf"
    local out="${FONT_DIR}/MesloLGS NF ${names[$i]}.ttf"
    curl -fsSL "$url" -o "$out"
  done
  ...
  warn "Set your terminal font to 'MesloLGS NF' in its preferences. Script can't do this for you."
}
